...
fn shm_name_fnv1a_matches_cpp() {
    ...
    assert_eq!(thoth_ipc::shm_name::fnv1a_64(b""), 0xcbf29ce484222325);

    // FNV-1a of "a" = 0xaf63dc4c8601ec8c
    assert_eq!(thoth_ipc::shm_name::fnv1a_64(b"a"), 0xaf63dc4c8601ec8c);

    // Verify make_shm_name prepends '/'
    let name = thoth_ipc::shm_name::make_shm_name("foo");
    assert_eq!(&name, "/foo");

    // Already has '/'
    let name2 = thoth_ipc::shm_name::make_shm_name("/bar");
    assert_eq!(&name2, "/bar");
}

...
CPP="$ROOT/cpp/libipc/build/bin/bounded_buffer"
RUST="$ROOT/rust/libipc/target/release/demo_bounded_buffer"
SWIFT="$ROOT/swift/libipc/.build/release/demo-bounded-buffer"
ZIG="$ROOT/zig/libipc/zig-out/bin/demo_bounded_buffer"
...
Build the four bounded-buffer demos first:
  (cd cpp/libipc  && cmake -B build -DLIBIPC_BUILD_DEMOS=ON . && cmake --build build --target bounded_buffer -j)
  (cd rust/libipc && cargo build --release --bin demo_bounded_buffer)
  (cd swift/libipc && swift build -c release --product demo-bounded-buffer)

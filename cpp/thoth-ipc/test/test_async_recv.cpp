// Layer 2 (THOTH_IPC_STDEXEC) tests for thoth::async_recv(): prove the senders/
...
std::string as_string(thoth::buff_t const &b) {
...
// value channel carries thoth::recv_result (message or recv_errc). set_error is
...
struct capture_receiver {
    ...
    thoth::recv_result *out;
    Env env;
    void set_value(thoth::recv_result r) noexcept { *out = std::move(r); code->store(1); }
...
template <class Env>
capture_receiver(std::atomic<int> *, thoth::recv_result *, Env) -> capture_receiver<Env>;

// A fake reactor modelling thoth::detail::reactor_like (no inheritance): captures
// the registered waiter so a test can drive on_ready() itself, and records
// add/remove calls. Demonstrates thoth::async_recv's dependency injection — no
// real kqueue/epoll thread involved.
struct fake_reactor {
    std::atomic<thoth::detail::reactor_waiter *> waiter{nullptr};
    ...
    void add(thoth::wait_handle_t, thoth::detail::reactor_waiter *w) {
        waiter.store(w);
        adds.fetch_add(1);
    }
    void remove(thoth::wait_handle_t, thoth::detail::reactor_waiter *w) {
        if (waiter.load() == w) waiter.store(nullptr);
        removes.fetch_add(1);
    }
};
static_assert(thoth::detail::reactor_like<fake_reactor>);
...
TEST(AsyncRecv, DeliversAlreadyQueuedMessage) {
    ...
    auto result = stdexec::sync_wait(thoth::async_recv(reader, pool.get_scheduler()));
    ASSERT_TRUE(result.has_value());                     // completed on value channel
    thoth::recv_result r = std::get<0>(std::move(*result));
...
TEST(AsyncRecv, CancellationCompletesStopped) {
    ...
    thoth::recv_result out;
    auto op = stdexec::connect(thoth::async_recv(reader, pool.get_scheduler()),
...
TEST(AsyncRecv, ManyChannelsOneReactor) {
    ...
    std::vector<std::unique_ptr<thoth::route>> readers, writers;
    for (int i = 0; i < N; ++i) {
        auto name = unique_name(("mux." + std::to_string(i)).c_str());
        readers.push_back(std::make_unique<thoth::route>(name.c_str(), thoth::receiver));
        writers.push_back(std::make_unique<thoth::route>(name.c_str(), thoth::sender));
    ...
    for (int i = 0; i < N; ++i) {
        auto result = stdexec::sync_wait(thoth::async_recv(*readers[i], pool.get_scheduler()));
        ASSERT_TRUE(result.has_value());
        thoth::recv_result r = std::get<0>(std::move(*result));
        ASSERT_TRUE(r.has_value());
...
TEST(AsyncRecv, InjectedReactorDrivesCompletion) {
    ...
    std::atomic<int> code{0};
    thoth::recv_result out;
    auto op = stdexec::connect(thoth::async_recv(reader, pool.get_scheduler(), fake),
                               capture_receiver{&code, &out, env});
    stdexec::start(op);
    ...
    EXPECT_EQ(disposition, thoth::detail::reactor_waiter::disposition::remove);
...
TEST(AsyncRecv, NoReadinessHandleYieldsErrc) {
    ...
    thoth::route sender_only{name.c_str(), thoth::sender};
    ASSERT_EQ(sender_only.native_wait_handle(), thoth::invalid_wait_handle);

    exec::static_thread_pool pool{1};
    auto result = stdexec::sync_wait(thoth::async_recv(sender_only, pool.get_scheduler()));
    ASSERT_TRUE(result.has_value());                     // completed on value channel
    thoth::recv_result r = std::get<0>(std::move(*result));
    ASSERT_FALSE(r.has_value());                         // carries an error code
    EXPECT_EQ(r.error(), thoth::recv_errc::no_readiness_handle);
}

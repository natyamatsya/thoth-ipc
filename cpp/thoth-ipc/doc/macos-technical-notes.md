...
(`src/thoth-ipc/platform/apple/semaphore_impl.h`):
...
**Fix:**
`src/thoth-ipc/platform/apple/mutex.h` — emulates timed lock with adaptive polling:
...
**Fix:**
PID-based liveness detection in `src/thoth-ipc/platform/apple/mutex.h`:
...
**Fix:**
`src/thoth-ipc/mem/new_delete_resource.cpp` — clamp alignment to `sizeof(void*)`:

...
fn run_generate(root: &Path, args: &[String]) {
    ...
    let (rendered, default_out) = match lang.as_str() {
        ...
        "protocols" => (gen_protocols(&abi), root.join("abi/protocols.txt")),
        other => {
            eprintln!("no generator for language '{other}' (have: zig, rust, swift, cpp, protocols)");
            std::process::exit(2);
        }
    };
...
/// A `protocol` tag says the field carries a coordination algorithm layout cannot
/// express, and names the conformance probe that must reproduce it everywhere.
fn protocol_note(comment: &str, probe: &str) -> String {
    format!("{comment} protocol: exercised by the `{probe}` conformance probe (byte-for-byte across ports)")
}

/// Flat listing of every protocol-bearing field, consumed by the xlang runner so
/// a new one without a probe is a hard error rather than an unnoticed gap.
fn gen_protocols(abi: &serde_json::Value) -> String {
    let mut o = String::new();
    o.push_str("# @generated by `tools/abi` from abi/abi.json — DO NOT EDIT.\n");
    o.push_str("# Regenerate: cargo run --manifest-path tools/abi/Cargo.toml -- generate --lang protocols\n");
    o.push_str("# <struct>.<field> <conformance-probe>\n");
    for s in abi["structs"].as_array().unwrap() {
        let name = s["name"].as_str().unwrap();
        for f in s["fields"].as_array().unwrap_or(&vec![]) {
            if let Some(p) = f["protocol"].as_str() {
                o.push_str(&format!("{name}.{} {p}\n", f["name"].as_str().unwrap()));
            }
        }
    }
    o

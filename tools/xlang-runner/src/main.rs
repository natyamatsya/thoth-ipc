...
fn main() -> ExitCode {
    ...
    // Every protocol the ABI declares must have a conformance probe. abi.json
    // tags the fields that carry a coordination algorithm (a lock discipline, a
    // free-list rule) with the probe that has to reproduce it; without this
    // check the tag would be one more piece of prose, which is exactly the
    // failure this machinery exists to prevent.
    // See context/abi-consistency-review.md.
    match protocols_declared(&cli.config) {
        Err(e) => eprintln!("note: protocol coverage unchecked: {e}"),
        Ok(declared) => {
            let missing: Vec<_> = declared
                .iter()
                .filter(|(_, probe)| !cfg.scenarios.conform.probes.contains(probe))
                .collect();
            if !missing.is_empty() {
                eprintln!("error: abi.json declares protocols with no conformance probe:");
                for (field, probe) in missing {
                    eprintln!("  {field} needs probe '{probe}' in [scenarios.conform].probes");
                }
                return ExitCode::from(2);
            }
        }
...
/// Read `abi/protocols.txt` (generated by `tools/abi`) as (field, probe) pairs.
/// Located relative to the config file so the runner needs no extra flag.
fn protocols_declared(config_path: &std::path::Path) -> Result<Vec<(String, String)>, String> {
    let root = config_path
        .parent()
        .and_then(|p| p.parent())
        .ok_or_else(|| "cannot locate repo root from config path".to_string())?;
    let path = root.join("abi/protocols.txt");
    ...
    Ok(text
        ...
        .filter_map(|l| {
            ...
            match (it.next(), it.next()) {
                (Some(field), Some(probe)) => Some((field.to_string(), probe.to_string())),
                _ => None,
            }
        })
        .collect())
